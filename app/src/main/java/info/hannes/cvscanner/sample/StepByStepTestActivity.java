...
//            fab.setVisibility(View.VISIBLE);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getExtras() != null)
            scanType = (CVCommand) getIntent().getExtras().getSerializable(EXTRA_SCAN_TYPE);
    ...
    double findAngleBetweenLines(Point firstSrc, Point firstDest, Point secSrc, Point secDest) {
        ...
        if (firstSrc.x == firstDest.x) //infinite slope
        {
            ...
            double secSlope = (secDest.y - secSrc.y) / (secDest.x - secSrc.x);
            if (firstSlope == 0)
                theta = Math.atan2(Math.abs(secDest.y - secSrc.y), Math.abs(secDest.x - secSrc.x));
            else if (secSlope == 0)
                theta = Math.atan2(Math.abs(firstDest.y - firstSrc.y), Math.abs(firstDest.x - firstSrc.x));
            else
                theta = Math.atan2(Math.abs(firstSlope - secSlope), Math.abs(1 + (firstSlope * secSlope)));
        }
    ...
    class CVTestMessage {
        ...
        CVTestMessage(CVCommand cmd, Mat input) {
            command = cmd;
            this.input = input;
        }
    }
    ...
    class CVTestRunner extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.obj instanceof CVTestMessage) {
                        ...
                        if (lines.rows() >= 3) {
                            ...
                            if (hLines.size() >= 2 && vLines.size() >= 2) {
                                ...
                                if ((nvLines.size() > 1 && nhLines.size() > 0) || (nvLines.size() > 0 && nhLines.size() > 1)) {
                                    ...
                                    for (Line l : nvLines) {
                                        if (l.length() / height < 0.60 || (left != null && right != null))
                                            break;
                                    ...
                                    for (Line l : nhLines) {
                                        if (l.length() / width < 0.60 || (top != null && bottom != null))
                                            break;
